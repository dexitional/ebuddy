...
module.exports = {
   ...
   fetchElectionDataByVoter : async (username:string) => {
      ...
      const { data:vt } = await db.from('eb_elector').select(`*`).eq('tag', username).eq('eb_centre.default', 1)
      ...
      if(vt && vt.length == 1){
        ...
        if(dm && dm.length > 0){
            ...
            for(var d of dm){
              ...
              const { data:res2 } = await db.from('eb_candidate').select(`*,eb_portfolio.name as portfolio,portfolio_id as pid`).eq('eb_candidate.status', 1).eq('eb_portfolio.election_id', d.id)
   ...
   activateCentre : async (cid:string) => {
      ...
      const { data:et } = await db.from('eb_centre').update({ default: 1 }).eq('id', cid).select()
      if(et) return et;
      return res;
   },
  ...
  loadCentrePhotos : async (cid:string) => {
   ...
   fetchElectionDataById : async (eid:string,tag:string) => {
      ...
      const { data:res2 } = await db.from('eb_candidate').select(`*,eb_portfolio.name as portfolio,portfolio_id as pid`).eq('eb_candidate.status', 1).eq('eb_portfolio.election_id', eid)
      if (res2 && res2.length > 0) data.candidates = res2;
      // Election data
